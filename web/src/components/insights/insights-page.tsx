  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <div className="grid gap-6 px-8 py-6 lg:grid-cols-2">
        ...
        {isLoading && <p className="text-sm text-[var(--muted)]">Loading insights</p>}
        {isError && (
          ...
          <>
            {data.summary_metrics && (
              <section className="surface-card p-6 lg:col-span-2 animate-fade-rise">
                <h2 className="font-editorial text-xl text-[var(--foreground-strong)]">Summary</h2>
                <dl className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  {Object.entries(data.summary_metrics).map(([key, val]) => (
                    <div key={key}>
                      ...
                      <dd className="mt-1 font-mono text-2xl text-[var(--foreground-strong)]">
            ...
            <section className="surface-card p-6 animate-fade-rise">
              ...
              <ul className="mt-4 space-y-3">
                {(data.rejection_patterns ?? []).length === 0 && (
                  <li className="text-sm text-[var(--muted)]">No rejection data yet.</li>
                )}
                {(data.rejection_patterns ?? []).map((p, index) => (
                  <li
                    key={p.reason}
                    className="stagger-item border-b border-[var(--border-subtle)] pb-3 last:border-b-0"
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-xs text-[var(--muted)]">{p.frequency}x</span>
                    </div>
                    <p className="mt-2 text-sm text-[var(--foreground)]">{p.suggested_action}</p>
                  </li>
                ))}
              </ul>
            </section>
            ...
            <section className="surface-card p-6 animate-fade-rise">
              ...
              <ul className="mt-4 space-y-3">
                {(data.success_patterns ?? []).length === 0 && (
                  <li className="text-sm text-[var(--muted)]">Need more reported keywords.</li>
                )}
                {(data.success_patterns ?? []).map((p, index) => (
                  <li
                    key={p.keyword_example}
                    className="stagger-item rounded-[var(--radius-sm)] bg-[var(--pastel-green-bg)] px-4 py-3"
                    style={{ ["--stagger-index" as string]: index }}
                  >
                    <p className="font-medium text-[var(--foreground-strong)]">{p.keyword_example}</p>
                    <p className="mt-1 font-mono text-xs text-[var(--muted)]">
                      ~{p.avg_views.toLocaleString()} views ·{" "}
                      {(p.avg_engagement_rate * 100).toFixed(1)}% engagement
                    </p>
                    <p className="mt-2 text-sm text-[var(--pastel-green-text)]">
            ...
            <section className="surface-card overflow-hidden lg:col-span-2 animate-fade-rise">
              ...
              {experimentsQuery.isLoading && (
                <p className="px-6 py-4 text-sm text-[var(--muted)]">Loading experiments</p>
              )}
              {experimentsQuery.isError && (
                <p className="px-6 py-4 text-sm text-[var(--pastel-red-text)]">
                  ...
                  <tbody>
                    {recentExperiments.map((item, index) => (
                      <tr
                        ...
                        style={{ ["--stagger-index" as string]: index }}
                      >
                        <td className="px-4 py-3.5 font-medium text-[var(--foreground-strong)]">
                          {item.keyword}
                        </td>
