...
export function DiscoveryProgressBar({ progress }: DiscoveryProgressBarProps) {
  ...
  return (
    <div
      className="w-full min-w-0 max-w-sm rounded-md border border-(--border) bg-(--surface-muted) px-3 py-2.5"
      role="progressbar"
    ...
    >
      ...
      <div className="mt-2.5 flex items-center gap-2.5">
        <div className="h-1.5 min-w-0 flex-1 overflow-hidden rounded-full bg-(--border)">
          <div
            className="h-full rounded-full bg-(--foreground-strong) transition-[width] duration-300"
            style={{ width: `${percent}%` }}
          />
        </div>
